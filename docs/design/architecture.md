...
**Leftover methods** (still in the tree, not the product loop): `sync`, `convert`, `getCommand` / `run`, skillsmith `export`. Do not extend them in this phase.
...
| windsurf | `.windsurf/skills/` | `.windsurf/commands/<name>.md` |
...
1. Windsurf / agents **command** paths — skills roots above are the scan contract; command-file dirs should be confirmed against current IDE docs when export is implemented.
2. One skill on many commands — the map allows it (`addSkill` / a second file). GUI this phase files from Inbox only.
3. npm name `skil` — may be taken. Product name is skil either way; bin alias can wait.
4. After we rewrite a stamped file, do we preserve a user-edited body? v1 no. Revisit if people use export as a round-trip editor.
5. Team YAML sync — keep or delete. Not in this loop.
