...
- **Import ≠ sync.** `sync()` still reads `.contextkit.yml` only. `importFromIDE('cursor' | 'claude')` scans one on-disk tree. Folder picker lives on the Sync tab (connect a project). Team-config copy stays a separate card.
